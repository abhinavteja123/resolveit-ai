  ...
  return (
    <div className="glass-card p-5 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-base font-semibold text-dark-200">Upload Runbook</h3>
      </div>

      {/* Drop zone */}
      <div
        onClick={() => fileInputRef.current?.click()}
        className="border-2 border-dashed border-dark-800/70 rounded-xl p-8 text-center cursor-pointer
                   hover:border-primary-600/40 hover:bg-primary-600/[0.02] transition-all duration-200"
      >
        ...
        {file ? (
          ...
          <div className="space-y-2">
            <Upload className="w-8 h-8 text-dark-700 mx-auto" />
            <p className="text-sm text-dark-500">Click to select a file</p>
            <p className="text-xs text-dark-700">PDF, DOCX, or TXT</p>
          </div>
        )}
      </div>
      ...
      {/* Category */}
      <div>
        <p className="text-[11px] font-semibold text-dark-600 uppercase tracking-widest mb-2">Category</p>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 border ${
                category === cat.value
                  ...
                  : 'bg-dark-900/50 text-dark-500 border-dark-800/60 hover:border-dark-700/60 hover:text-dark-400'
              }`}
            >
              {cat.label}
            </button>
      ...
      <button onClick={handleUpload} disabled={!file || uploading} className="btn-primary w-full flex items-center justify-center gap-2">
        {uploading ? <><Loader2 className="w-4 h-4 animate-spin" /> Indexing…</> : <><Upload className="w-4 h-4" /> Upload & Index</>}
      </button>
